...
def gui_agregar_stock():
    producto = simpledialog.askstring('Agregar stock', 'Nombre del producto:')
    if not producto:
        return
    ...
    if producto not in df['producto'].values:
        ...
        return
    cantidad = simpledialog.askinteger('Agregar stock', 'Cantidad a agregar:', minvalue=1)
    if cantidad is None:
        return
    df.loc[df['producto'] == producto, 'stock'] += cantidad
    df.to_csv(stocksito, index=False)
    messagebox.showinfo('Éxito', f'Stock actualizado de {producto}')
...
def gui_agregar_nuevo_producto():
    ...
    if nombre in df['producto'].values:
        messagebox.showerror('Error', 'Este producto ya existe')
        return
...
def gui_abrir_cli():
    ...
    t = threading.Thread(target=entrada_trabajador, daemon=True)
    t.start()
...
def launch_tkinter_gui():
    ...
    btn_products = tk.Button(frame, text='Mostrar productos', width=25, command=gui_mostrar_productos)
    btn_products.grid(row=0, column=0, padx=5, pady=5)

    btn_prices = tk.Button(frame, text='Mostrar precios', width=25, command=gui_mostrar_precios)
    btn_prices.grid(row=0, column=1, padx=5, pady=5)

    btn_add_stock = tk.Button(frame, text='Agregar stock', width=25, command=gui_agregar_stock)
    btn_add_stock.grid(row=1, column=0, padx=5, pady=5)

    btn_add_product = tk.Button(frame, text='Agregar nuevo producto', width=25, command=gui_agregar_nuevo_producto)
    btn_add_product.grid(row=1, column=1, padx=5, pady=5)

    btn_plot_stock = tk.Button(frame, text='Graficar stock', width=25, command=graficar_stock_productos)
    btn_plot_stock.grid(row=2, column=0, padx=5, pady=5)

    btn_plot_sales = tk.Button(frame, text='Graficar ventas', width=25, command=graficar_por_categoria_venta)
    btn_plot_sales.grid(row=2, column=1, padx=5, pady=5)

    btn_cli = tk.Button(frame, text='Abrir CLI', width=52, command=gui_abrir_cli)
    btn_cli.grid(row=3, column=0, columnspan=2, padx=5, pady=10)

    root.mainloop()


if __name__ == '__main__':
    # Por defecto lanzamos la GUI; si prefieres la interfaz por consola, ejecuta `entrada_trabajador()` manualmente.
    launch_tkinter_gui()